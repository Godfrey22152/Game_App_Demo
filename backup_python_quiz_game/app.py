...
def question(question_num):
    ...
    return render_template('question.html', question_num=question_num, question=question_key,
                           options=options[question_key], next_question_num=question_num + 1)
...
def results():
    ...
    options = [questions[q]['options'] for q in shuffled_questions[:10]]

    # Zip the question, options, correct answer, and user answer
    question_answer_pairs = list(zip([q['question'] for q in questions.values()], options, correct_answers, user_answers[:10]))
